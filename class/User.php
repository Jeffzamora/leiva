...
class User
{
	...
	public function login()
	{
		if ($this->email && $this->password) {
			...
			if ($result->num_rows > 0) {
				...
				$userId = $user['id'];
				$insertQuery = "INSERT INTO logs (id_user, log_in) VALUES (?, now())";
	...
	{
		$sqlQuery = "SELECT l.id, u.first_name, u.last_name, u.email, l.log_in, l.log_out, TIMEDIFF(l.log_out, l.log_in) AS connection_time
					 FROM logs l
					 INNER JOIN " . $this->userTable . " u ON l.id_user = u.id";
	
		if (!empty($_POST["search"]["value"])) {
			$sqlQuery .= ' WHERE (l.id LIKE "%' . $_POST["search"]["value"] . '%" ';
			...
			$sqlQuery .= ' OR l.log_in LIKE "%' . $_POST["search"]["value"] . '%" ';
			$sqlQuery .= ' OR l.log_out LIKE "%' . $_POST["search"]["value"] . '%" ';
			$sqlQuery .= ' OR TIMEDIFF(l.log_out, l.log_in) LIKE "%' . $_POST["search"]["value"] . '%" )';
		}
	
		if (!empty($_POST["order"])) {
			$sqlQuery .= ' ORDER BY ' . $_POST['order']['0']['column'] . ' ' . $_POST['order']['0']['dir'] . ' ';
		} else {
			$sqlQuery .= ' ORDER BY l.id DESC ';
		}
		...
		$stmtTotal = $this->conn->prepare("SELECT COUNT(*) AS total FROM logs");
		$stmtTotal->execute();
		...
		while ($log = $result->fetch_assoc()) {
			$rows = array();
			$rows[] = $count;
			$rows[] = ucfirst($log['first_name']) . " " . ucfirst($log['last_name']);
			$rows[] = $log['email'];
			$rows[] = $log['log_in'];
			$rows[] = $log['log_out'];
	
			$logOut = $log['log_out'];
			$connectionTime = $log['connection_time'];
	
			if ($logOut === null) {
				$logOut = '<span style="color: red;">Sin salida</span>';
			}
	
			if ($connectionTime === null) {
				$connectionTime = '<span style="color: red;">Sin salida</span>';
			}
